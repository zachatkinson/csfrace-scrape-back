...
class TestRedisAdvanced:
    ...
    async def test_unicode_and_special_characters(self, redis_cache):
        ...
        test_cases = [
            ...
            (
                "https://test.com/special",
                "<html>Special: \"quotes\" 'apostrophes' &amp; entities</html>",
            ),
    ...
    async def test_redis_configuration_validation(self):
        ...
        configs_to_test = [
            # Valid config
            {"redis_host": "localhost", "redis_port": 6379, "redis_db": 0, "should_work": True},
            # Invalid host
            {
            ...
            # Invalid port
            {"redis_host": "localhost", "redis_port": 99999, "redis_db": 0, "should_work": False},
        ]
...
async def test_redis_fallback_behavior():
    ...
    if REDIS_AVAILABLE:
        ...
        config = CacheConfig(
            backend=CacheBackend.REDIS, redis_host="nonexistent.example.com", redis_port=6379
        )
