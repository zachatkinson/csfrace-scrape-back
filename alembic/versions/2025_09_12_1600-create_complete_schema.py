...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_webauthn_challenges_challenge_key'), 'webauthn_challenges', ['challenge_key'], unique=True)
    ...
    op.create_table(
        ...
        sa.Column('user_id', sa.String(length=50), nullable=False, comment='User ID who is locked out'),
        sa.Column('username', sa.String(length=100), nullable=False, comment='Username for quick lookups and audit trail'),
        sa.Column('failed_attempts', sa.Integer(), server_default='0', nullable=False, comment='Current number of consecutive failed login attempts'),
        sa.Column('is_locked', sa.Boolean(), server_default='false', nullable=False, comment='Current lockout status'),
        sa.Column('lockout_reason', sa.String(length=100), nullable=True, comment='Reason for lockout (failed_attempts, suspicious_activity, admin_action)'),
        sa.Column('first_failed_attempt_at', sa.DateTime(timezone=True), nullable=True, comment='Timestamp of first failed attempt in current sequence'),
        sa.Column('last_failed_attempt_at', sa.DateTime(timezone=True), nullable=True, comment='Timestamp of most recent failed attempt'),
        sa.Column('locked_at', sa.DateTime(timezone=True), nullable=True, comment='Timestamp when account was locked'),
        sa.Column('locked_until', sa.DateTime(timezone=True), nullable=True, comment='Timestamp when lockout expires (NULL for permanent)'),
        sa.Column('unlocked_at', sa.DateTime(timezone=True), nullable=True, comment='Timestamp when account was unlocked'),
        sa.Column('client_ip', sa.String(length=45), nullable=True, comment='IP address of failed login attempts'),
        sa.Column('user_agent', sa.Text(), nullable=True, comment='User agent of failed login attempts'),
        sa.Column('unlocked_by', sa.String(length=100), nullable=True, comment='Admin user who unlocked the account manually'),
    ...
    op.create_table(
        'revoked_tokens',
        sa.Column('jti', sa.String(length=100), nullable=False, comment='JWT ID claim - unique identifier for each token'),
        sa.Column('user_id', sa.String(length=50), nullable=True, comment='User who owned the token (for bulk revocation)'),
        sa.Column('token_type', sa.String(length=20), server_default='access', nullable=False, comment='Token type: access, refresh, or reset'),
        sa.Column('issued_at', sa.DateTime(timezone=True), nullable=False, comment='When token was originally issued'),
        sa.Column('expires_at', sa.DateTime(timezone=True), nullable=False, comment='When token naturally expires'),
        sa.Column('revoked_at', sa.DateTime(timezone=True), nullable=False, comment='When token was revoked'),
        sa.Column('revocation_reason', sa.String(length=100), nullable=True, comment='Reason for revocation: logout, password_change, admin_action, etc.'),
        sa.Column('revoked_by', sa.String(length=50), nullable=True, comment='Who/what revoked the token (user_id, admin_id, or system)'),
        sa.Column('client_ip', sa.String(length=45), nullable=True, comment='IP address when token was revoked'),
        sa.Column('user_agent', sa.String(length=500), nullable=True, comment='User agent when token was revoked'),
