...
async def health_stream() -> StreamingResponse:
    """Health SSE endpoint with full service data."""
...
@router.get("/simple")
async def simple_health_stream() -> StreamingResponse:
    """Simple SSE endpoint without dependencies."""

    async def generate():
        yield 'event: connection\ndata: {"type": "connection", "message": "connected"}\n\n'
        yield 'event: service-update\ndata: {"service": "frontend", "status": "healthy"}\n\n'
