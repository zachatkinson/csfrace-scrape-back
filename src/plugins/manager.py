...
class PluginManager:
    ...
    async def _initialize_plugin(self, plugin_name: str) -> None:
        ...
        try:
            ...
            logger.info("Plugin initialized", name=plugin_name, type=plugin_config.plugin_type.value)
    ...
    def _build_pipeline(self) -> None:
        ...
        for plugin_type in self._pipeline:
            self._pipeline[plugin_type].sort(
                key=lambda name: self._plugins[name].config.priority
            )
        
        logger.debug("Built plugin execution pipeline", pipeline={
            ptype.value: plugins for ptype, plugins in self._pipeline.items()
        })
    ...
    async def execute_pipeline(
        ...
        logger.debug("Executing plugin pipeline", 
        ...
        for plugin_name in plugins:
            ...
            try:
                ...
                context.record_plugin_stats(plugin_name, {
                    'duration': duration,
                    'success': True,
    ...
    async def process_content(
        ...
        try:
            ...
            data = await self.execute_pipeline(
                PluginType.METADATA_EXTRACTOR,
                data,
                context
            )
