...
from .challenge_storage import IChallengeStorage, get_challenge_storage
...
class WebAuthnService:
    ...
    def __init__(
        ...
        challenge_storage: IChallengeStorage | None = None,
    ):
        ...
        # Use injected challenge storage or get default (InMemory for single-instance, Redis for multi-instance)
        self._challenge_storage = challenge_storage or get_challenge_storage()

    async def generate_registration_options(
        self, user: User
    ) -> tuple[WebAuthnRegistrationOptions, str]:
        ...
        # NOTE: This function generates its own challenge internally
        ...
        # Use the challenge generated by the library (not our own)
        challenge = bytes_to_base64url(registration_options.challenge)

        # Store challenge for verification using async storage
        challenge_key = f"reg_{user.id}_{challenge}"
        await self._challenge_storage.store_challenge(
            challenge_key,
            {
                "challenge": challenge,
                "user_id": user.id,
        ...
        credential_opts = RegistrationCredentialOptions(
            ...
            authenticator_selection={
                "userVerification": (
                    registration_options.authenticator_selection.user_verification.value
                    if registration_options.authenticator_selection
                    and registration_options.authenticator_selection.user_verification
                    else "preferred"
                )
            },
    ...
    async def verify_registration_response(
        ...
        challenge_data = await self._challenge_storage.get_challenge(challenge_key)
        ...
        # Clean up challenge
        await self._challenge_storage.delete_challenge(challenge_key)
    ...
    async def generate_authentication_options(
        ...
        credentials = self._get_user_credentials(user.id) if user else []
        ...
        challenge_key = f"auth_{user.id}_{challenge}" if user else f"auth_any_{challenge}"
        ...
        options = WebAuthnAuthenticationOptions(
            ...
            user_verification=(
                authentication_options.user_verification.value
                if authentication_options.user_verification
                else "preferred"
            ),
        )
    ...
    async def cleanup_expired_challenges(self, max_age_minutes: int = 10) -> int:
        """Cleanup expired challenges - Maintenance operation."""
        return await self._challenge_storage.cleanup_expired(max_age_minutes)
...
class PasskeyManager:
    ...
    async def start_passkey_registration(
        ...
        options, challenge_key = await self.webauthn_service.generate_registration_options(user)
    ...
    async def start_passkey_authentication(self, user: User | None = None) -> dict[str, Any]:
        """Start passkey authentication flow - Supports usernameless login."""
        options, challenge_key = await self.webauthn_service.generate_authentication_options(user)
