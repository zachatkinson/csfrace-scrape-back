...
class AsyncImageDownloader:
    ...
    def __init__(self, output_dir: Path, max_concurrent: int = config.max_concurrent_downloads):
        ...
        logger.debug("Initialized image downloader", output_dir=str(output_dir), max_concurrent=max_concurrent)
    ...
    async def _download_image(self, session: aiohttp.ClientSession, url: str) -> str:
        ...
        try:
            ...
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=30)) as response:
                ...
                logger.debug("Successfully downloaded image", url=url, filename=filename, size=response.content_length)
                return filename
