...
from .base import BaseCacheBackend, CacheConfig, CacheBackend, CacheEntry
...
class CacheManager:
        ...
        try:
            ...
            if self.config.backend == CacheBackend.FILE:
                ...
                if not REDIS_AVAILABLE:
                    raise ValueError("Redis backend requested but redis package not available. Install with: pip install redis")
    ...
    async def set_metadata(self, url: str, metadata: Dict[str, Any], ttl: Optional[int] = None) -> bool:
    ...
    async def set_robots_txt(self, domain: str, robots_content: str, ttl: Optional[int] = None) -> bool:
