...
class RedisEventStore(EventStore):
    ...
    For production use with horizontal scaling.
    Uses Redis Streams for efficient event storage and retrieval.

    Following Single Responsibility Principle - only manages Redis event storage.
    """

    def __init__(self, redis_url: str, max_length: int = 10000):
        """
        Initialize Redis event store.

        Args:
            redis_url: Redis connection URL
            max_length: Maximum number of events to keep in Redis stream
        """
        self._redis_url = redis_url
        self._max_length = max_length
        # TODO: Initialize Redis connection when Redis is available
        # For now, use in-memory as fallback
        self._fallback_store = InMemoryEventStore(max_events=max_length)
        logger.warning(
            "redis_event_store_not_implemented",
            message="Using in-memory store as fallback",
        )

    async def store(self, event: Event) -> None:
        """Store event in Redis stream."""
        # TODO: Implement Redis stream storage
        await self._fallback_store.store(event)
    ...
    async def get_recent(
        ...
        # TODO: Implement Redis stream retrieval
        return await self._fallback_store.get_recent(limit, event_type, since)

    async def get_by_id(self, event_id: UUID) -> Event | None:
        """Get event by ID from Redis."""
        # TODO: Implement Redis event lookup
        return await self._fallback_store.get_by_id(event_id)

    async def clear(self) -> None:
        """Clear Redis stream."""
        # TODO: Implement Redis stream clearing
