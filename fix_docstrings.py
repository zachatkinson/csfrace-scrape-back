        ...
        if line_num <= len(lines):
            ...
            if match:
                ...
                if original_text.startswith("Decorator factory"):
                    ...
                    if not any(original_text.lower().startswith(verb) for verb in 
                             ["get", "set", "create", "update", "delete", "handle", "process", "perform", "return", "load", "save", "check", "validate", "convert", "transform"]):
                        new_text = "Handle " + original_text.lower()
                    else:
                        return False
            ...
            if "def __init__(" in method_line:
                ...
                if args_match:
                    ...
                    args = [arg.strip().split(':')[0].split('=')[0].strip() 
                           for arg in args_str.split(',') if arg.strip() and arg.strip() != 'self']
                    
                    if args:
                        args_doc = '\n            '.join([f"{arg}: {arg.replace('_', ' ').title()} parameter" for arg in args])
                    else:
